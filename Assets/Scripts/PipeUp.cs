...
public class PipeUp : MonoBehaviour {
	...
	void Update () {
		if (character.transform.position.x - transform.position.x > 30) {
			...
			Instantiate (gameObject, new Vector2 (character.transform.position.x + 30 + xRan, -6 + yRan), transform.rotation);
			Instantiate (pipeDown, new Vector2 (character.transform.position.x + 30 + xRan, 6 + gapRan + yRan), transform.rotation);
			Destroy (gameObject);
		}
